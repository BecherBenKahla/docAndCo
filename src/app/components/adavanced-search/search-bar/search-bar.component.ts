...
export class SearchBarComponent implements OnInit {
  ...
  @Output() search: EventEmitter<{ whoSearchText: string, whereSearchText: string }> = new EventEmitter();
  ...
  onSearch() {
    if (typeof (this.searchTerm) === 'string') {
      this.newItemEvent.emit({
        whoAreaUsed: true,
        whereAreaUsed: true,
        whoSearchText: this.searchTerm,
        whereSearchText: this.location,
        sectionUsed: "",
        showDetail: false,
        dataPerson: {}
      });
  ...
  getData(dataOption: any) {
    ...
    if (this.persons.includes(dataOption)) {
      ...
      item.dataPerson = {
        ...
        location: dataOption.medicalStructureName,
      ...
      item.specialtyId = dataOption.medicalSpecialtyId;
    } else {
      item.whoSearchText = dataOption.name;
